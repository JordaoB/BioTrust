...
class RiskEngine:
    ...
    WEIGHT_AMOUNT = 0.30      # 30%
    WEIGHT_LOCATION = 0.25    # 25%
    WEIGHT_TIME = 0.20        # 20%
    WEIGHT_BEHAVIOR = 0.15    # 15%
    WEIGHT_TYPE = 0.10        # 10%
    ...
    def _calculate_amount_risk(self, amount: float, user_profile: Dict) -> float:
        ...
        # Calcular score baseado no ratio
        if ratio <= 1.0:
            ...
            # 2x a 5x a média
            score = 40 + (ratio - 2.0) * 15
        else:
            # Mais de 5x a média
            score = 85 + min((ratio - 5.0) * 3, 15)
        
        # Valores absolutos muito altos sempre pontuam alto
        if amount > 1000:
            score = max(score, 60)
        if amount > 5000:
            score = max(score, 80)
