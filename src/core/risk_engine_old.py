...
class RiskEngine:
    ...
    THRESHOLD_LOW_RISK = 35     # 0-35: Aprovação direta
    THRESHOLD_HIGH_RISK = 75    # 75-100: Alto risco + bloqueio
    
    # ========== CONSTANTES DE RISCO ==========
    IMPOSSIBLE_TRAVEL_DISTANCE = 100  # km - Distância suspeita em curto tempo
    HIGH_AMOUNT_THRESHOLD = 500       # €500+ = Agravamento automático
    LOW_AMOUNT_THRESHOLD = 20         # €20- = Risco muito baixo
    RAPID_TX_COUNT = 3                # 3+ transações em 2h = Pânico
    RAPID_TX_WINDOW_HOURS = 2         # Janela de tempo para velocity check
    SAME_RECIPIENT_LIMIT = 3          # 3+ envios para mesmo destinatário em 1h
    ...
    def analyze_transaction(self, transaction: Dict, user_history: Dict = None) -> Dict:
        """
        ...
        Args:
            transaction: Dicionário com dados da transação:
                ...
                - timestamp: datetime
                - recipient_email: str (para transferências)
                - merchant_id: str (para pagamentos)
                - user_profile: dict (dados históricos do utilizador)
            
            user_history: Dict com histórico recente:
                - recent_transactions: List[Dict] (últimas transações)
                - last_transaction_location: Dict
    ...
    def _calculate_amount_risk(self, amount: float, user_profile: Dict) -> float:
        ...
        # Valores absolutos altos - MAIS RIGOROSO
        if amount > 300:
            score = max(score, 50)   # €300+ começa a ser suspeito
        if amount > 500:
            score = max(score, 60)   # €500+ suspeito
        if amount > 1000:
